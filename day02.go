...
func checkPasswordPolicy(line string) bool {
	var regEx = `^(?P<min>\d+)-(?P<max>\d+) (?P<letter>[a-z]): (?P<psswd>[a-z]+)$`
	...
	res := count >= min && count <= max
	if !res {
		fmt.Printf("line %s is invalid\n", line)
	}
	return res
}

func solveDay02Example() {
	examples := []string{
		"1-3 a: abcde",
		"1-3 b: cdefg",
		"2-9 c: ccccccccc",
	...
	for _, line := range examples {
		if checkNewPasswordPolicy(line) {
			validCount++
		}
	}
...
func solveDay02Part1() {
	lines := getDay02Data()
	validCount := 0
	for _, line := range lines {
		if checkPasswordPolicy(line) {
			validCount++
		}
	}
	fmt.Printf("There are %d valid passwords\n", validCount)
...
func checkNewPasswordPolicy(line string) bool {
	...
	count := 0
	if passwd[idx1-1] == letter {
		count++
	}
	if passwd[idx2-1] == letter {
		count++
	}
